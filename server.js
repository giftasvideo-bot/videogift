...
// page â€” separate login from the full admin dashboard. Set these in your
...
// -- LOGIN (admin or postcard-staff â€” same endpoint, different accounts) --
...
app.patch('/api/gift/:id/buyer', requireAuth, requireAdmin, async (req, res) => {
  ...
  // Default to QR-only since that's the primary product â€” a physical
...
//
// LOCK: once a card has a printed_at, it's locked â€” one QR code, one print.
...
app.get('/api/admin/cards', requireAuth, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      // Don't fail the whole card list over it â€” retry with just id.
      console.warn('âš ï¸ /api/admin/cards: full select failed, retrying without buyer/design fields:', error.message);
